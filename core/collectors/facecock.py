    ...
    try:
        ...
        for item in items:
            ...
            if region_match:
                ...
                # '[대전-서구]도솔체육관' 처럼 대괄호 뒤에 텍스트가 붙는 경우
...
def collect_tournaments(max_pages: int = 3):
    ...
    out_path = os.path.join(RAW_TOURNAMENT_DIR, f"facecok_tournaments_{timestamp}.json")
    with open(out_path, "w", encoding="utf-8") as f:
        json.dump(final_list, f, ensure_ascii=False, indent=4)
        
    print(f"[*] 페이스콕 대회 총 {len(final_list)}개 저장 완료: {out_path}")
