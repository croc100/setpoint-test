...
def collect_tournaments():
    ...
    out_path = os.path.join(RAW_TOURNAMENT_DIR, f"nearminton_tournaments_{timestamp}.json")
    
    with open(out_path, "w", encoding="utf-8") as f:
        json.dump(final_list, f, ensure_ascii=False, indent=4)
        
    print(f"[*] 우동배 대회 총 {len(final_list)}개 저장 완료: {out_path}")
